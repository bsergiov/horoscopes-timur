...
@section('content')
    <h2>EN all articles</h2>
    {{--    <h5>Count {{count($articles) ?? 0}}</h5>--}}
    <form action="{{route('en.store')}}" method="POST">
        @csrf
        <div class="mb-1">
    ...
    @foreach($articles  as $value)
        <div class="card mt-2">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <a href="{{route('en.edit', $value['id'])}}" class="btn btn-outline-primary">Edit</a>
                    <form class="ml-1" action="{{route('en.destroy', $value['id'])}}" method="POST">
                        @method('DELETE')
                        @csrf
